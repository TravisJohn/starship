...
- If `intentContext` is present, it is the project's already-answered Intent Ledger (purpose, success criteria, accepted tradeoffs, what it must never become). Use it only to check that the current answer stays aligned with that stated intent, and to gently flag it in your reply when a requirement seems to drift from or contradict the ledger. Never let it override or invent details for what the builder is currently answering.

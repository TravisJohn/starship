...
5. **Collapse repeats.** A `reasoningClusters` entry already represents every work item that shared one rationale - extract ONE decision from it. Never emit one decision per repeated item.
...
Optional, default to `null`/`false` - never guess:
...
Do not include a `collapsed` field - that count is computed afterward from `reasoningClusters` directly, not from anything you report.
...
{"decisions":[{"chose":"...","over":"...","because":"...","evidence":[{"source":"log","file":"...","anchor":"..."}],"servesIntent":"purpose"|"successCriteria"|"acceptedTradeoffs"|"neverDo"|null,"reversible":"cheap"|"load-bearing"|null,"supersedesChose":null,"isRecapOnly":false}]}

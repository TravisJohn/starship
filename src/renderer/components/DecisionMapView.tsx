...
import type { DecisionMapGenerateResponse } from "../../shared/ipc";
import { LoadingAnimation } from "./LoadingAnimation";

type DecisionMapViewProps = {
  projectId: string;
  projectPath: string;
  projectName: string;
};
 ...
 * in-app-only view.
 */
export const DecisionMapView = ({
  projectId,
  projectPath,
  projectName
}: DecisionMapViewProps): JSX.Element => {
  const [result, setResult] = useState<DecisionMapGenerateResponse | null>(null);
  ...
  useEffect(() => {
    ...
    void window.starship.decisionMap
  ...
  const download = async (): Promise<void> => {
    ...
    setSavedPath(null);
    try {
      const response = await window.starship.decisionMap.download({
        html: result.html,
        projectName
      });
  ...
  return (
    <section className="flex h-full min-h-0 flex-col bg-zinc-950 text-zinc-100">
      ...
      {result ? (
        <iframe
          title={`${projectName} decision map`}
          srcDoc={result.html}

...
type DecisionMapOverlayProps = {
  project: MissionProject | null;
  onClose: () => void;
};

export const DecisionMapOverlay = ({
  project,
  onClose
}: DecisionMapOverlayProps): JSX.Element | null => {
  useEffect(() => {
    if (!project) {
      return;
    }
  ...
  return (
    ...
    >
      <section
        role="dialog"
        aria-modal="true"
        aria-labelledby="decision-map-title"
      ...
      >
        <div className="flex h-11 shrink-0 items-center justify-between gap-3 border-b border-zinc-800 px-4">
          <h2 id="decision-map-title" className="truncate text-sm font-semibold">
            {project.name}
          </h2>
          <button
            type="button"

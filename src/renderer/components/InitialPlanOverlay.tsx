...
import { InitialPlanView } from "./InitialPlanView";

type InitialPlanOverlayProps = {
  project: MissionProject | null;
  onClose: () => void;
};

export const InitialPlanOverlay = ({
  project,
  onClose
}: InitialPlanOverlayProps): JSX.Element | null => {
  useEffect(() => {
    if (!project) {
      return;
    }
  ...
  return (
    ...
    >
      <section
        role="dialog"
        aria-modal="true"
        aria-labelledby="initial-plan-title"
      ...
      >
        <div className="flex h-11 shrink-0 items-center justify-between gap-3 border-b border-zinc-800 px-4">
          <h2 id="initial-plan-title" className="truncate text-sm font-semibold">
            {project.name}
          </h2>
          <button
            type="button"
        ...
        <div className="min-h-0 flex-1">
          <InitialPlanView projectPath={project.path} projectName={project.name} />
        </div>
      </section>
    </div>
  );

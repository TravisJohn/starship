...
import type { GitTreeGenerateResponse } from "../../shared/ipc";
...
export const GitTreeView = ({
  ...
  projectPath,
  projectName
}: GitTreeViewProps): JSX.Element => {
  const [result, setResult] = useState<GitTreeGenerateResponse | null>(null);
  ...
  const download = async (): Promise<void> => {
    ...
    setSavedPath(null);
    try {
      const response = await window.starship.gitTree.download({
        html: result.html,
        projectName
      });
    ...
    <section className="flex h-full min-h-0 flex-col bg-zinc-950 text-zinc-100">
      <header className="flex shrink-0 items-center justify-between gap-3 border-b border-zinc-800 px-4 py-3">
        <div className="min-w-0">
          <h2 className="truncate text-sm font-semibold leading-none">Git Tree</h2>
          <p className="mt-1 truncate text-xs text-zinc-500">
            {result ? `${result.commitCount} commits` : projectName}
          </p>
        </div>
        <button
          type="button"

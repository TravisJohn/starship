...
type GitTreeOverlayProps = {
  project: MissionProject | null;
  onClose: () => void;
};

export const GitTreeOverlay = ({ project, onClose }: GitTreeOverlayProps): JSX.Element | null => {
  useEffect(() => {
    if (!project) {
      return;
    }
  ...
  return (
    ...
    >
      <section
        role="dialog"
        aria-modal="true"
        aria-labelledby="git-tree-title"
      ...
      >
        <div className="flex h-11 shrink-0 items-center justify-between gap-3 border-b border-zinc-800 px-4">
          <h2 id="git-tree-title" className="truncate text-sm font-semibold">
            {project.name}
          </h2>
          <button
            type="button"

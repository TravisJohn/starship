...
import { NarrativeJourneyView } from "./NarrativeJourneyView";

type NarrativeJourneyOverlayProps = {
  project: MissionProject | null;
  onClose: () => void;
};

export const NarrativeJourneyOverlay = ({
  project,
  onClose
}: NarrativeJourneyOverlayProps): JSX.Element | null => {
  useEffect(() => {
    if (!project) {
      return;
    }
  ...
  return (
    ...
    >
      <section
        role="dialog"
        aria-modal="true"
        aria-labelledby="narrative-journey-title"
        className="flex h-[82vh] w-full max-w-6xl min-w-0 flex-col overflow-hidden rounded-md border border-zinc-700 bg-zinc-950 text-zinc-100 shadow-2xl"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="flex h-11 shrink-0 items-center justify-between gap-3 border-b border-zinc-800 px-4">
          <h2 id="narrative-journey-title" className="truncate text-sm font-semibold">
            {project.name}
          </h2>
          <button
            type="button"

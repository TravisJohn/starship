...
export const NotesOverlay = ({ project, onClose }: NotesOverlayProps): JSX.Element | null => {
  useEffect(() => {
    if (!project) {
      return;
    }
  ...
  return (
    ...
    >
      <section
        ...
        className="flex h-[70vh] w-full max-w-md min-w-0 flex-col overflow-hidden rounded-md border border-zinc-700 bg-zinc-950 text-zinc-100 shadow-2xl"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="flex h-11 shrink-0 items-center justify-between gap-3 border-b border-zinc-800 px-4">
          <h2 id="notes-title" className="truncate text-sm font-semibold">
            {project.name} — Notes
          </h2>
          <button
            type="button"

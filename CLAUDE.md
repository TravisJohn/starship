...
1. **Zero agency.** Starship launches Claude Code, observes `~/.claude` state, and explains at decision altitude. Never add code that edits user projects, acts autonomously, or answers prompts on Claude's behalf.
   - **Amendment, 2026-08-07 (deliberate, approved by Travis).** Exactly one carve-out: Starship may write **`CONTINUITY.md` at an existing project's root** — always overwritten, never read back as a source of truth, never any other path. It exists so a session ended via Exit & Summarize still leaves a handoff for whichever agent picks the project up next, including the case where the finishing agent hit a usage limit and could not write one itself. Everything else in a user's project remains untouchable; this exception does not generalise to "Starship may write files now."
